...
angular.module('uiBreadcrumbs', ['ui.router'])
    ...
    .directive('uiBreadcrumbs', function($interpolate, $state) {
        return {
            restrict: 'E',
            templateUrl: 'views/directives/uiBreadcrumbs.tpl.html',
            scope: {
                displaynameProperty: '@'
            },
            link: function(scope) {
                ...
                function getIconClass(currentState) {
                    var propertyReference;
                    var icon_fa;

                    propertyReference = currentState;
                    if (angular.isDefined(propertyReference.data['icon_fa'])) {
                        if (propertyReference.data['icon_fa'] === false) {
                            return false;
                        } else {
                            propertyReference = propertyReference.data['icon_fa'];
                        }
                    } else {
                        return '';
                    }
                    icon_fa = $interpolate(propertyReference)(currentState.locals.globals);
                    return icon_fa;
                }
            }
        };
